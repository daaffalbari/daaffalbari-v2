...
import type { CmsPostMeta } from "@/lib/posts";

interface CmsPostContentProps {
  meta: CmsPostMeta;
  children: React.ReactNode; // rendered Markdoc body
}

export function CmsPostContent({ meta, children }: CmsPostContentProps) {
    ...
    <>
      ...
      <article className="min-h-screen pt-[calc(var(--nav-height)+var(--space-xl))] pb-[var(--space-3xl)]">
        <div className="page">
          ...
          >
            {meta.tags.length > 0 && (
              <p className="eyebrow">{meta.tags.slice(0, 3).join(" · ")}</p>
            )}
